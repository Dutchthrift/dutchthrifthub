...
export function NotesPanel({ entityType, entityId, currentUser, className }: NotesPanelProps) {
  ...
  const isTwoColumn = className?.includes('notes-two-column');

  return (
    <div className={cn(isTwoColumn ? "" : "space-y-4", className?.replace('notes-two-column', ''))} data-testid="notes-panel">
      {isTwoColumn ? (
        // 2-column layout: composer left, notes right
          ...
          <div className="space-y-3">
              ...
              {uniqueAuthors.length > 0 && (
                <Select value={authorFilter || "all"} onValueChange={(val) => setAuthorFilter(val === "all" ? undefined : val)}>
                  <SelectTrigger className="w-[150px] h-9" data-testid="filter-author">
                    <SelectValue placeholder="Alle auteurs" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {selectedTagFilters.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-muted-foreground">Actieve tag filters:</span>
            ...
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-medium text-muted-foreground" data-testid="notes-count">
                {filteredNotes.length} {filteredNotes.length === 1 ? "notitie" : "notities"}
                {hasActiveFilters && notes.length !== filteredNotes.length && (
                  <span className="text-xs ml-1">(gefilterd van {notes.length})</span>
                )}
              </h3>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {deletedNotes.length > 0 && (
              <div className="space-y-2">
                <h4 className="text-xs font-medium text-red-600 dark:text-red-400 uppercase tracking-wide">
                  Verwijderd ({deletedNotes.length})
                </h4>
            ...
            {notes.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                <p>Nog geen notities. Voeg er hierboven een toe.</p>
              </div>
            )}
          </div>
        </>
