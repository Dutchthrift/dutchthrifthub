...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // SSE endpoint for order import with real-time progress
  app.get("/api/shopify/import-orders-progress", async (req, res) => {
    ...
    try {
      sendProgress({ type: 'start', message: 'Starting order import...' });

      // Step 1: Sync customers first
      sendProgress({ type: 'status', message: 'Fetching customers from Shopify...' });
      const shopifyCustomers = await shopifyClient.getAllCustomers();
      
      sendProgress({ 
        type: 'customers_fetched', 
        total: shopifyCustomers.length,
        message: `Found ${shopifyCustomers.length} customers` 
      });
      ...
      sendProgress({ 
        ...
        message: `Processed ${customerCount} customers` 
      });

      // Step 2: Fetch and count orders
      sendProgress({ type: 'status', message: 'Fetching orders from Shopify...' });
      const shopifyOrders = await shopifyClient.getAllOrders();
      
      const totalOrders = shopifyOrders.length;
      sendProgress({ 
        type: 'orders_fetched', 
        total: totalOrders,
        message: `Found ${totalOrders} orders to import` 
      });
      ...
      // Step 3: Process orders with progress updates
      let processed = 0;
      let created = 0;
      let updated = 0;
      ...
      for (const shopifyOrder of shopifyOrders) {
        try {
          ...
          // Send progress update every 10 orders or on last order
          if (processed % 10 === 0 || processed === totalOrders) {
            sendProgress({ 
              ...
              created,
              updated,
              message: `Importing order ${processed} / ${totalOrders}...` 
            });
          }
        } catch (error) {
