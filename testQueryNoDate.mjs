...
async function testQueryNoDate() {
    try {
        ...
        // Query WITHOUT date, only status
        ...
        if (result.errors) {
            ...
            for (const edge of result.data.orders.edges) {
                console.log(`   Order ${edge.node.name} - Status: ${edge.node.returnStatus} - Updated: ${edge.node.updatedAt}`);
            }
        }
